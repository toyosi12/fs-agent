...
def evaluate_application(
    ...
    for i, tc in enumerate(ui_test_cases):
        if i > 0:
            time.sleep(5)
        score = score_frontend_test(llm, task_instruction, tc, frontend_code)
    ...
    for i, tc in enumerate(backend_test_cases):
        if i > 0 or ui_test_cases:
            time.sleep(5)
        score = score_backend_test(llm, task_instruction, tc, backend_code)
    ...
    for i, ds in enumerate(data_structures):
        if i > 0 or backend_test_cases or ui_test_cases:
            time.sleep(5)
        score = score_database_test(llm, task_instruction, ds, migration_sql)
    ...
    if frontend_code.strip():
        if data_structures or backend_test_cases or ui_test_cases:
            time.sleep(5)
        result.appearance = score_appearance(llm, task_instruction, frontend_code)
